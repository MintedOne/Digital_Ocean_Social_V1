// Browser-only imports - prevent server-side execution
let ffmpeg: any = null;
...
async function loadFFmpegModules() {
  ...
  if (!FFmpegModule) {
    ...
    FFmpegModule = ffmpegModule.FFmpeg;
    fetchFile = utilModule.fetchFile;
    toBlobURL = utilModule.toBlobURL;
  }
  
  return { FFmpegModule, fetchFile, toBlobURL };
}
...
export async function initFFmpeg(onProgress?: (progress: number) => void): Promise<any> {
  try {
    ...
    const { FFmpegModule, toBlobURL: toBlobURLFunc } = await loadFFmpegModules();
    
    ffmpeg = new FFmpegModule();
    ...
    const coreURL = await toBlobURLFunc(`${baseURL}/ffmpeg-core.js`, 'text/javascript');
    const wasmURL = await toBlobURLFunc(`${baseURL}/ffmpeg-core.wasm`, 'application/wasm');
...
export async function mergeVideoWithOutro(
  ...
  try {
    ...
    const { fetchFile: fetchFileFunc } = await loadFFmpegModules();
    
    // Write input files
    console.log('📝 Writing main video to FFmpeg filesystem...');
    await ffmpegInstance.writeFile('main.mp4', await fetchFileFunc(mainVideo));
    
    console.log('📝 Writing outro video to FFmpeg filesystem...');
    await ffmpegInstance.writeFile('outro.mp4', await fetchFileFunc(outroVideo));
...
export async function applyVideoMetadata(
  ...
  // Write input file
  await ffmpegInstance.writeFile('input.mp4', await fetchFileFunc(video));
